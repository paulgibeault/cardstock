...
/* ------------------------------------------------------------------ *
 * Which input may end a held beat (#176, #181)
 * ------------------------------------------------------------------ */
...
// and it is here.
//
// AND IT IS ASKED OF EVERY BEAT THAT WAITS NOW, NOT ONLY THE TRICK (#181). The
// counts of a cribbage show wait for a person at the same rung, and each one of
// them opens inside the dispatch of the tap that dismissed the one before it —
// so the rule generalises without changing: a beat stamps when it opened, and an
// input from before that moment belongs to the beat that has already gone.
// Without it one gesture would cascade trick → count → count → count → sheet and
// a hand would end in a single tap. The numbers below are still two numbers.
...
test("the input that opened a trick hold does not end it", () => {
  ...
  assert.strictEqual(inputEndsHeldBeat(HOLD_AT, HOLD_AT - 0.4), false,
    "the tap that played the fourth card swept the trick it had just posed");
  assert.strictEqual(inputEndsHeldBeat(HOLD_AT, HOLD_AT - 180), false,
...
test("an input from after the hold opened does end it", () => {
  ...
  assert.strictEqual(inputEndsHeldBeat(HOLD_AT, HOLD_AT + 0.1), true);
  assert.strictEqual(inputEndsHeldBeat(HOLD_AT, HOLD_AT + 900), true);
});
...
test("an input stamped at the exact moment of the hold is the opening gesture", () => {
  ...
  assert.strictEqual(inputEndsHeldBeat(HOLD_AT, HOLD_AT), false);
});
...
test("an unreadable timestamp ends the hold rather than trapping the player in it", () => {
  ...
  for (const stamp of [undefined, null, NaN, "1500", Infinity]) {
    assert.strictEqual(inputEndsHeldBeat(HOLD_AT, stamp), true,
  ...
  for (const opened of [undefined, null, NaN]) {
    assert.strictEqual(inputEndsHeldBeat(opened, 6_000), true,
