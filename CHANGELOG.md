...
One entry per issue, newest first. Each entry is the build record for that
issue: what changed, why the code reads the way it does, and what verifying it
turned up.

These entries were written in `IMPLEMENTATION_NOTES.md` and moved here
unedited. That file's other half — the milestone 1 status snapshot, the bugs
simulation caught, the known limitations and the bot-tuning findings — is now
[docs/notes/IMPLEMENTATION_NOTES.md](docs/notes/IMPLEMENTATION_NOTES.md), and
the plans the work was done from are under
...
That is docs/plans/FEEDBACK_INBOX item 22's second half — "on turns where I was leading a
...
anyway (see the cribbage entry, #107), so a remote client could not light the
