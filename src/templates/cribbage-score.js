...
// WHAT THE PARTS ARE CALLED USED TO LIVE HERE. `partPhrase`, `namedParts` and
// `scoredParts` name the `kind`s a `showScored` part can carry, and the platform
// printed them by importing this file into src/ui/showCard.js — one template's
// module, named in a platform file (#219). They live beside the OTHER emitter of
// that event now (`heldParts`, src/engine/scoring.js), where the platform can
// read them without knowing which game is being played, and this file keeps the
// one import its header promises.
