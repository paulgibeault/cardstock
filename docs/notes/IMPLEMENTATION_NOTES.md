...
- **Arcade integration (added 2026-08-04, `docs/plans/ARCADE_ENHANCEMENTS.md` v2
...
`docs/plans/ARCADE_ENHANCEMENTS.md` was rewritten as the Cardstock-side
...
full elsewhere — `docs/plans/MULTIPLAYER_PLAN.md` for the wire, `docs/plans/TABLES_PLAN.md` for
...
A twelve-item pass over how the table FEELS, planned in `docs/plans/UX_PASS_PLAN.md`
...
Everything above is milestone 1: the status snapshot, what simulation caught,
the two known limitations, and the bot-tuning findings. The per-issue build
record that used to follow it — one entry per issue from #101 onward — is
[CHANGELOG.md](../../CHANGELOG.md), newest first. The plans the work was done
from are in [docs/plans/](../plans/), each marked shipped.

## Next steps

The Phase 8 multiplayer, per-pack `theme.css` and card-face work this section
used to list all shipped; see the CHANGELOG and
[docs/plans/MULTIPLAYER_PLAN.md](../plans/MULTIPLAYER_PLAN.md) /
[docs/plans/TABLES_PLAN.md](../plans/TABLES_PLAN.md) for what landed. What is
open now:

- **The consolidation pass (#201)** — the `Consolidation pass` milestone: the
  architecture audit's bug, mechanical, structural and split-up tiers. This
  document's own restructuring is part of it (#222).
- **The review workstream (#188)** — [REVIEW_PLAN.md](../../REVIEW_PLAN.md):
  the end-of-hand hold, the scrollable match map, and the "play from here"
  sandbox. Phases 1–3 shipped; the lobby door, the open lens, the sandbox and
  the shared view are still open.
- **Poker (#108)** — the last pack in the new-games ladder (#100), blocked on
  the betting-model decisions rather than on code.
